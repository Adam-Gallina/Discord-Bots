...
class FishData:
    def __init__(self, name, minSize, maxSize, baseValue, fishClass, rarity, image):
        self.name = name
...
def FishToEmbed(embed, name, size, value):
    return embed.add_field(name=f'{name} ({size} inches)', value=value, inline=False)
